	...
	return this.ready.then(function() {
		...
		var res = evaluate(function evaluate(fnstr, hasCb, args) {
				...
				var done = function done(err, res) {
					__horseman.cbargs = __horseman.cbargs || {
						err: err,
						iserr: err instanceof Error,
						res: res
					};
				};
				try {
			...
			.then(function waitForCb(done) {
				if (done !== true) {
					debug('.evaluate() waiting for callback', fname, done);
					return waitForPage.call(self, page, function waitForCb() {
						return !!__horseman.cbargs;
					}, true);
				}
				debug('.evaluate() finished synchronously', fname, done);
			})
