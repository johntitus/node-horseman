 ...
 * @param {object|object[]} arg - Cookie or array of cookies
 */
...
exports.evaluate = function(fn) {
	...
	debug.apply(debug, ['.evaluate()', fn.name || '<anonymous>'].concat(args));
	return this.ready.then(function() {
		...
		return HorsemanPromise.fromCallback(function(done) {
			return evaluate(function evaluate(fnstr, hasCb, args) {
				...
				.then(function waitForCb(done) {
					if (done !== true) {
						debug('.evaluate() waiting for async function');
					...
					debug('.evaluate() finished synchronously');
				})
...
exports.html = function(selector) {
	debug('.html()', selector);
	return this.__evaluate(function html(selector) {
		if (selector) {
			...
			return (window.jQuery) ?
					jQuery('html').html() : document.documentElement.innerHTML;
		}
	}, selector);
};
...
function waitForPage(page, fn) {
	...
	debug.apply(debug,
			['.waitFor()', fn.name || '<anonymous>'].concat(args.slice(2)));
	return this.ready.then(function() {
		return new HorsemanPromise(function(resolve, reject) {
			var start = Date.now();
