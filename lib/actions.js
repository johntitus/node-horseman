...
/**
 * Get or set the settings for Phantom's current page.
 * @param {Object} [settingsObject] - Timeout, in mili-seconds, to use.
 * As detailed here - http://phantomjs.org/api/webpage/property/settings.html
 */
exports.pageSettings = function(settingsObject) {
	var self = this;
	return this.ready.then(function() {
		return new HorsemanPromise(function(resolve, reject) {
			if (!settingsObject) {
				self.page.get('settings', function(err, settings) {
					debug('.pageSettings() get');
				...
				self.page.get('settings', function(err, settings) {
					...
					self.page.set('settings', settings, function(err) {
						if (err) { return reject(err); }
						debug('.pageSettings() set', settingsObject);
						return resolve();
					});
				});
			}
...
/**
 * Get or set the resourceTimeout for Phantom.
 * @param {int} [timeout] - Timeout, in mili-seconds, to use.
 */
exports.resourceTimeout = function(timeout) {
	var self = this;
	return this.ready.then(function() {
		return new HorsemanPromise(function(resolve, reject) {
			if (!userAgent) {
				self.page.get('settings', function(err, settings) {
					debug('.resourceTimeout() get');
					if (err) { return reject(err); }
					return resolve(settings.resourceTimeout);
				});
			} else {
				self.page.get('settings', function(err, settings) {
					if (err) { return reject(err); }
					settings.resourceTimeout = resourceTimeout;
					self.page.set('settings', settings, function(err) {
						if (err) { return reject(err); }
						debug('.resourceTimeout() set', resourceTimeout);
						return resolve();
					});
				});
			}
