...
/**
 * Click on a random link and fire a 'click event'.
 * @param {string} selector
 */
exports.clickRand = function() {
  var self = this;

  debug('.clickRand()');
  return self.__evaluate(function clickRand() {
    var element;
    var event;
    ...
    for(var i = 0; i<length; i++){
      item = a[i];
      if("nofollow" !== item.getAttribute("rel") && "_blank" !== item.getAttribute("target") && checkHref(item.getAttribute("href"))){
        links.push(item);
      }
    }
    element = links[rand(0, links.length-1)];
		event = document.createEvent('MouseEvent');
		event.initEvent('click', true, true);
		element.dispatchEvent(event);
    ...
    function checkHref(href){
      if(!href || href == "#page" || href == "#" || "javascript:;" == href.toLocaleString() || "javascript:void(0)" == href.toLocaleString() ){
        return false;
      }
      return true;
    }
  })
  .tap(function() { debug('.clickRand() done'); });
};
