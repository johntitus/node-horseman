...
exports.includeJs = function(url) {
	...
	return this.ready.then(function() {
		return new HorsemanPromise(function(resolve, reject) {
			debug('.includeJs()', url);
			self.page.includeJs(url, resolve);
		});
	});
};
...
exports.evaluate = function(fn) {
	...
	var hasCb = args.length < fn.length; // If fn takes a callback

	// TODO: Handle the current page changing?
	return this.ready.then(function() {
		return HorsemanPromise.fromCallback(function(done) {
			return HorsemanPromise.fromCallback(function(done) {
					return self.page.evaluate(function wrappedFn(fnstr, hasCb, args) {
						window.__horseman.cbargs = undefined;
						var done = function done() {
							window.__horseman.cbargs = window.__horseman.cbargs || arguments;
						};
						try {
							var fn;
							eval('fn = ' + fnstr);
							...
							if (hasCb) {
								...
								if (p && typeof p.then === 'function') {
									...
									// node-phantom-simple can't have p undefined
									return done(null, p === undefined ? null : p);
								}
							}
						} catch (err) {
							return done(err);
						}
					}, fn.toString(), hasCb, args, done);
				})
				.then(function waitForCb() {
					return self.waitFor(function waitForCb() {
						return !!window.__horseman.cbargs;
					}, true);
				})
				.then(function handleCb() {
					return self.__evaluate(function handleCb() {
						// Have to turn arguments Object into a real Array
						return Array.prototype.slice.call(window.__horseman.cbargs);
					});
				})
				.spread(done)
				.catch(done);
...
exports.__evaluate = function(fn) {
	var args =  Array.prototype.slice.call(arguments, 1);
	var self = this;
	...
	return this.ready.then(function() {
		return HorsemanPromise.fromCallback(function(done) {
			return HorsemanPromise.fromCallback(function(done) {
				...
				return self.page.evaluate(function(fnstr, args) {
					try {
						var fn;
						eval('fn = ' + fnstr);
...
exports.visible = function(selector) {
	...
	var vis = this.__evaluate(function(selector) {
		if (window.jQuery) {
				...
				var waiting = setInterval(function() {
					if (self.waitingForNextPage === false) {
						...
						if (diff > self.options.timeout) {
							...
							reject(new TimeoutError('timeout duing .waitForNextPage()'));
						}
					}
...
exports.waitFor = function() {
	debug('.waitFor()', arguments[0].name || '<anonymous>', value);
	var self = this;
		...
		return new HorsemanPromise(function(resolve, reject) {
				...
				var checkInterval = setInterval(function() {
					...
					if (diff > self.options.timeout) {
						...
						reject(new TimeoutError('timeout during .waitFor()'));
					} else {
						self
							.__evaluate.apply(self, args)
							.then(function(res) {
								if (res === value) {
