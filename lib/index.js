...
// Internals
//**************************************************//
Horseman.prototype.untilOnPage = function(check, value) {
	var self = this;
	var condition = false;
...
Horseman.prototype.afterNextPageLoad = function() {
	...
	var self = this;
	return self.untilOnPage(isUnloaded,true)
		.then(function(){
			return self.untilOnPage(isLoaded,true);
		})
};
 ...
 */
function until(fun, timeout, interval) {
	var start = Date.now();
	return new Promise( function( resolve, reject ){
		...
		var checker = setInterval(function() {
			...
			var checkFunc = fun();
			checkFunc.then(function( res ){
				
				if (res || diff > timeout) {
					clearInterval(checker);
					if ( res ) resolve( res );
					else reject('Timeout passed.');
				} else {
					return;
				}
			});
