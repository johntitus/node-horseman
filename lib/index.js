...
Horseman.prototype.pageMaker = function(url, _page) {
	...
	return HorsemanPromise.try(function getPage() {
		var page;
		if (_page) {
			page = setupPage(_page);
		} else {
			page = HorsemanPromise.fromCallback(function(done) {
				return self.phantom.createPage(function(err, page) {
					if (page) {
						return setupPage(page).asCallback(done);
					}
					return done(err, page);
				});
			});
		}
		return page.tap(function attachPage(page) {
			if (!_page || self.options.switchToNewTab) {
				self.page = page;
			}
			var tabNum = self.tabs.push(page) - 1;
			self.onTabCreated(page, tabNum);
		});
	});
	...
	function setupPage(page) {
		...
		// Create Promise for when onLoadFinished callback is done
		var loadDone;
		var load = HorsemanPromise.fromCallback(function(done) {
			loadDone = done;
		});

		return HorsemanPromise
			.fromCallback(function(done) {
				page.set('settings.resourceTimeout', self.options.timeout, done);
			})
			.then(function openPage() {
				if (_page) {
					return;
				}
			...
			.then(function waitForLoadFinished() {
				return load;
			})
			.then(function() {
				loadDone = undefined;
